# Fork from https://github.com/cortesi/modd

[![Travis Build Status](https://travis-ci.org/caeret/modd.svg?branch=master)](https://travis-ci.org/caeret/modd)
...
page](https://github.com/caeret/modd/releases/latest), download the package for
...
Alternatively, with Go 1.17+ installed, you can install `modd` directly using `go install`. Please note that CGO is required, so if you happen to have it disabled you will need to prepend the `CGO_ENABLED=1` environment variable.
    ...
    $ go install github.com/caeret/modd/cmd/modd@latest
